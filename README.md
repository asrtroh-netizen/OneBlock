...
1. 新建一个公开 GitHub 仓库，例如 `adobe-rules`。
2. 把本压缩包里的所有内容上传到仓库根目录，包含隐藏目录 `.github`。
3. 打开仓库的 **Actions** 页面。
...
假设仓库为 `asrtroh-netizen/adobe-rules`：

```text
https://raw.githubusercontent.com/asrtroh-netizen/adobe-rules/main/adobe-block.json
```
...
rule-providers:
  Adobe-Block:
    ...
    behavior: classical
    format: yaml
    url: "https://raw.githubusercontent.com/asrtroh-netizen/adobe-rules/main/adobe-block.yaml"
